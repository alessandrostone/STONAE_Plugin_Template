...
enum class PID { GainWet, Frequency, NumParams };
static constexpr int NumParams = static_cast<int>(PID::NumParams);

enum class Unit { Db, Hz, NumUnits };
...
inline String toName(PID pID) {
  ...
  case PID::Frequency:
    return "Frequency";
  case PID::NumParams:
    return "Num Params"; // Handle NumParams explicitly
  default:
    return "Unknown";
  }
}
...
inline String toID(PID pID) { return toID(toName(pID)); }
...
inline String toString(Unit unit) {
  ...
  case Unit::Hz:
    return "hz";
  case Unit::NumUnits:
    return "Unknown"; // Handle NumUnits explicitly
  }
}
...
namespace valToStr {
inline ValToStr db() {
  return [](float val, int) { return String(val, 2) + " dB"; };
}
...
inline void createParam(UniqueRAPVector &vec, PID pID, const RangeF &range,
  ...
  case Unit::Hz:
    ...
    break;
  case Unit::NumUnits:
    valToStrFunc = [](float, int) { return "Unknown"; };
    strToValFunc = [](const String &) { return 0.0f; };
    break;
  }

  vec.push_back(std::make_unique<APF>(
      id, name, range, defaultVal, toString(unit),
      APPCategory::genericParameter, valToStrFunc, strToValFunc));
}

inline Layout createParameterLayout() {
  UniqueRAPVector params;

  createParam(params, PID::GainWet, range::lin(-120.f, 12.f), 0.f, Unit::Db);
  createParam(params, PID::Frequency, range::withCentre(20.f, 20000.f, 1000.f),
              1000.f, Unit::Hz);
